...
describe('Create Campaign Theme', () => {
    ...
    it('Create Campaign theme with valid API and method using super admin permission', () => {
        const accessToken = Cypress.env('accessToken');
        // Load the file before making the request
        cy.fixture('Bee.png', 'binary').then((fileContent) => {
            const blob = Cypress.Blob.binaryStringToBlob(fileContent, 'image/png');
    
            const formData = new FormData();
            formData.append('file', blob, 'Bee.png'); // Append file with filename
