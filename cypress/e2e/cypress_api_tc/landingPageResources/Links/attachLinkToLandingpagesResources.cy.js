...
let resellerVersion;
describe('Attach Links on Pages and Resources.', () => {
    before(() => {
        cy.request({
            method: 'POST',
        ...
        cy.then(() => {
            ...
            headers: {
                ...
                cy.log('Login Response:', JSON.stringify(response));
                expect(response.status).to.eql(200, 'Check for status of Linkss');
            });
        });
        cy.log('Get Ids of Linkss From Available Linkss');
        cy.then(() => {
            const accessToken = Cypress.env('accessToken');
            cy.request({
                method: 'GET',
    ...
    it('Attach Page Links with Invalid api and Valid method', () => {
        const accessToken = Cypress.env('accessToken');
        cy.request({
            method: 'GET',
            url: `${baseConfig.baseUrl}/sites/${baseConfig.siteId}/pages/${baseConfig.sitePageId}/linksss/${linkId}`,
            headers: {
                Authorization: `Bearer ${accessToken}`,
    ...
    it('Attach Page Links with valid api and InValid method', () => {
        const accessToken = Cypress.env('accessToken');
        cy.request({
            method: 'PUT',
    ...
    it('Attach Page Links with Invalid api and InValid method', () => {
        const accessToken = Cypress.env('accessToken');
    ...
    it('Attach Page Links with Valid api and Valid method', () => {
        const accessToken = Cypress.env('accessToken');
        cy.request({
            method: 'POST',
    ...
    it('Attach Page Global Links with MasterAdmin Permission', () => {
    ...
    it('Attach Page Resources with Reseller Admin Permission', () => {
        ...
        cy.log('Add the Site Linkss on the Reseller Site')
        cy.then(() => {
        ...
        cy.log('Fetch the Available List from Links')
        cy.then(() => {
            const accessTokenResellerAdmin = Cypress.env('accessTokenResellerAdmin');
            cy.request({
                method: 'GET',
        ...
        cy.log('Attach the Links To Pages and Resources.')
        cy.then(() => {
            const accessTokenResellerAdmin = Cypress.env('accessTokenResellerAdmin');
            cy.request({
                method: 'POST',
